...
source=(
    ...
    "${_srcname}.tar.gz::https://gitlab.com/xdevs23/linux-nitrous/-/archive/v${_kernver}/linux-nitrous-v${_kernver}.tar.gz"
    "config"
    "auto-cpu-optimization.sh"
    "$pkgbase::git+https://github.com/clearlinux-pkgs/linux.git"
    "https://gitlab.com/xanmod/linux-patches/-/raw/master/linux-${_major}.y-xanmod/wine/0001-winesync-Introduce-the-winesync-driver-and-character.patch"
    "https://gitlab.com/xanmod/linux-patches/-/raw/master/linux-${_major}.y-xanmod/futex/0001-futex-Add-entry-point-for-FUTEX_WAIT_MULTIPLE-opcode.patch"
...
prepare() {
    mv ${srcdir}/linux-nitrous-v${_kernver} $_srcname
    cd ${srcdir}/$_srcname
    ...
    for i in $(grep '^Patch' ${srcdir}/$pkgbase/linux.spec |\
     grep -Ev '^Patch0132|^Patch0118|^Patch0113|^Patch0138|^Patch0139' | sed -n 's/.*: //p'); do
        ...
        patch -Np1 -i "$srcdir/$pkgbase/${i}" || true
    done
